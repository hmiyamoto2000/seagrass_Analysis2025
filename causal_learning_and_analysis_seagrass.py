...
本解析では、変数間の因果構造を明らかにするために、制約ベースのPCアルゴリズムを使用して因果有向非巡回グラフ（DAG）を推定。
連続変数については、まずシャピロ・ウィルク検定により正規性を評価し、非正規分布が疑われる変数が存在する場合は、ノンパラメトリックな独立性検定（Kernel-based Conditional Independence Test: KCIT）を活用。
離散変数が含まれる場合はカイ二乗検定を、すべての変数が正規分布に従うと判断された場合はFisher-z検定を適用。
因果グラフは NetworkX および matplotlib により可視化。
"""
